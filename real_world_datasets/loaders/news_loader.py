...
def load_news(n_items=None):
    """
    Load news rankings from the full_rankings.csv file.
    
    Args:
        n_items (int, optional): Number of items to keep from the rankings.
                                 If None, keeps all items (10).
    
    Returns:
        list: List of rankings, where each ranking is a list of item IDs (1-based indexing).
    """
    # Path to the CSV file (in cache directory)
    csv_path = Path(__file__).parent / "cache_files" / "news" / "full_rankings.csv"
    n_items_to_keep = n_items
    ...
    print(f"✓ Loaded {len(rankings):,} news rankings ({len(rankings[0]) if rankings else 0} items each)")
