...
def fetch_and_process_data(file_name):
    ...
    if response.status_code == 200:
        ...
        # we slice off the last empty piece with [:-1], also the starting column is a 1 so we slice of the first too.
        votes = [list(map(int, line.split(',')))[1:-1] for line in data[n+2 : n+2+a]]
