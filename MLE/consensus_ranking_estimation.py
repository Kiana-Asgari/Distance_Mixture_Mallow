...
#     # Using absolute difference (L1 norm)
...
def _consensus_ranking_estimation(pis: np.ndarray) -> np.ndarray:
    """
    Given an (m × n) array `pis`, whose rows are m sampled permutations of {1,…,n},
    return the permutation σ̂ₘ that minimises
        Σ_{i=1}^{n} Σ_{l=1}^{m} |π^{(l)}(i) − σ(i)|.
    ...
    positions = np.arange(1, n + 1)
    # Using indicator function instead of absolute difference
    C = (pis[:, :, None] != positions).sum(axis=0)   # (n, n)
    # TODO: I changes this from np.abs to indicator function
