...
def psi_m_wrapper_no_alpha(x, pis, sigma, Delta, alpha, ord): 
    lookup_data = load_lookup_tables(len(sigma), Delta=Delta)
    psi = psi_m(pis, sigma,alpha, x[0], lookup_data)
    print('alpha', x[0], 'err', np.linalg.norm(psi, ord=ord))
...
def psi_m_wrapper(x, pis, sigma, Delta, ord=1/2): 
    ...
    print('alpha', x[0], 'beta', x[1], 'err', np.linalg.norm(psi, ord=ord))
