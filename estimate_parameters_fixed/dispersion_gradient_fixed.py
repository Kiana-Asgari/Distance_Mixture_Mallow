...
def gradient_dispersion_parametes(permutation_samples,
    ...
    emp_displacement, emp_log_displacement = compute_displacement_empirical(permutation_samples,
                                                                            ...
                                                                            alpha,
                                                                            verbose=verbose)
    grad_theta = 1/theta*(- emp_displacement + m * exp_displacement)
    
    grad_alpha =  (-np.log(theta) * emp_log_displacement + m * np.log(theta)* log_displacement) 
    return 1/m * grad_alpha - lambda_reg * alpha, \
           1/m * grad_theta + lambda_reg * theta #normalize by m
